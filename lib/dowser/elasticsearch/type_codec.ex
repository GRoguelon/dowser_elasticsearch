defmodule Dowser.Elasticsearch.TypeCodec do
  @moduledoc """
  ...
  needed. Field-level casting itself is delegated to
  `Dowser.Elasticsearch.Codec`; mappings are fetched (and cached) through
  ...
  def decode(term, opts) do
    ...
    if Keyword.get(opts, :source, false) do
      ...
      {:ok, Mappable.decode(term, mapping, key_fn, &Codec.load/2)}
    else
      {:ok, Mappable.decode(term, mapping_fn(config), key_fn, &Codec.load/2)}
    end
  end
  ...
  def encode(term, opts) do
    ...
    cond do
      ...
      true ->
        {:ok, Mappable.encode(term, fetch_mapping(config, index), &Codec.dump/2, false)}
    end
  end
  ...
  defp encode_doc_key(%{} = term, doc_key, mapping) do
    ...
    dump_fn = fn value -> Mappable.encode(value, mapping, &Codec.dump/2, false) end
  ...
  defp bulk_step(item, {:payload, _action, index}, config, _default_index) do
    {Mappable.encode(item, fetch_mapping(config, index), &Codec.dump/2, false), :header}
  end
