defmodule Dowser.Elasticsearch.CodecTest do
  use ExUnit.Case, async: true

  alias Dowser.Elasticsearch.Codec

  doctest Dowser.Elasticsearch.Codec

  describe "decode/2" do
    test "casts date strings and epoch millis to DateTime" do
      assert Codec.decode("2026-08-11T00:00:00.000Z", %{
               "type" => "date",
               "format" => "strict_date_optional_time"
             }) == ~U[2026-08-11 00:00:00.000Z]

      assert Codec.decode(0, %{"type" => "date", "format" => "epoch_millis"}) ==
               ~U[1970-01-01 00:00:00.000Z]

      assert Codec.decode("2026-08-11T00:00:00.000Z", %{
               "type" => "date_nanos",
               "format" => "strict_date_optional_time"
             }) == ~U[2026-08-11 00:00:00.000Z]
    end

    test "casts ip strings to :inet tuples" do
      assert Codec.decode("127.0.0.1", %{"type" => "ip"}) == {127, 0, 0, 1}
    end

    test "casts binary fields from Base64" do
      assert Codec.decode(Base.encode64("raw"), %{"type" => "binary"}) == "raw"
    end

    test "casts geo_point objects to {lat, lon} tuples" do
      assert Codec.decode(%{"lat" => 1.2, "lon" => 3.4}, %{"type" => "geo_point"}) ==
               {1.2, 3.4}
    end

    test "casts date_range objects to Date.Range structs" do
      assert Codec.decode(%{"gte" => "2026-08-01", "lte" => "2026-08-11"}, %{
               "type" => "date_range",
               "format" => "strict_date"
             }) == Date.range(~D[2026-08-01], ~D[2026-08-11])
    end

    test "casts integer_range objects to Range structs" do
      assert Codec.decode(%{"gte" => 1, "lte" => 10}, %{"type" => "integer_range"}) == 1..10
    end

    test "an integer_range with non-integer bounds passes through unchanged" do
      assert Codec.decode(%{"gte" => 1.5, "lte" => 2.5}, %{"type" => "integer_range"}) ==
               %{"gte" => 1.5, "lte" => 2.5}
    end

    test "an unrecognized value passes through unchanged" do
      assert Codec.decode("not a date", %{"type" => "date"}) == "not a date"
    end

    test "a mapping with no format defaults to Elasticsearch's own default (strict_date_optional_time||epoch_millis)" do
      assert Codec.decode("2026-08-11", %{"type" => "date"}) == ~D[2026-08-11]

      assert Codec.decode("2026-08-11T00:00:00.000Z", %{"type" => "date"}) ==
               ~U[2026-08-11 00:00:00.000Z]

      assert Codec.decode(0, %{"type" => "date"}) == ~U[1970-01-01 00:00:00.000Z]
    end

    test "an unmatched type falls back to identity" do
      assert Codec.decode("hello", %{"type" => "text"}) == "hello"
    end

    test "nil short-circuits" do
      assert Codec.decode(nil, %{"type" => "date"}) == nil
    end
  end

  describe "encode/2" do
    test "dumps DateTime to ISO-8601" do
      assert Codec.encode(~U[2026-08-11 00:00:00Z], %{
               "type" => "date",
               "format" => "strict_date_optional_time"
             }) == "2026-08-11T00:00:00Z"
    end

    test "a mapping with no format defaults to Elasticsearch's own default (strict_date_optional_time||epoch_millis)" do
      assert Codec.encode(~D[2026-08-11], %{"type" => "date"}) == "2026-08-11"

      assert Codec.encode(~U[2026-08-11 00:00:00Z], %{"type" => "date"}) ==
               "2026-08-11T00:00:00Z"
    end

    test "dumps :inet tuples to strings" do
      assert Codec.encode({127, 0, 0, 1}, %{"type" => "ip"}) == "127.0.0.1"
    end

    test "dumps raw binaries to Base64" do
      assert Codec.encode("raw", %{"type" => "binary"}) == Base.encode64("raw")
    end

    test "dumps {lat, lon} tuples to geo_point objects" do
      assert Codec.encode({1.2, 3.4}, %{"type" => "geo_point"}) ==
               %{"lat" => 1.2, "lon" => 3.4}
    end

    test "dumps Date.Range structs to date_range objects" do
      assert Codec.encode(Date.range(~D[2026-08-01], ~D[2026-08-11]), %{
               "type" => "date_range",
               "format" => "strict_date"
             }) == %{"gte" => "2026-08-01", "lte" => "2026-08-11"}
    end

    test "dumps Range structs to integer_range objects" do
      assert Codec.encode(1..10, %{"type" => "integer_range"}) == %{"gte" => 1, "lte" => 10}
    end

    test "a non-Range value for integer_range passes through unchanged" do
      assert Codec.encode("not a range", %{"type" => "integer_range"}) == "not a range"
    end

    test "an unmatched type falls back to identity" do
      assert Codec.encode("hello", %{"type" => "text"}) == "hello"
    end

    test "nil short-circuits" do
      assert Codec.encode(nil, %{"type" => "date"}) == nil
    end
  end

  describe "a codec of your own" do
    defmodule CustomCodec do
      @behaviour Dowser.Elasticsearch.Codec

      @impl true
      def decode(value, %{"type" => "scaled_float", "scaling_factor" => factor})
          when is_integer(value) do
        value / factor
      end

      # `date` is already handled by the built-in codec; matching it first
      # replaces that cast.
      def decode(value, %{"type" => "date"}), do: {:raw, value}

      def decode(value, field), do: Dowser.Elasticsearch.Codec.decode(value, field)

      @impl true
      def encode(value, %{"type" => "scaled_float", "scaling_factor" => factor})
          when is_float(value) do
        round(value * factor)
      end

      def encode(value, field), do: Dowser.Elasticsearch.Codec.encode(value, field)
    end

    test "a delegated type still uses the built-in codec" do
      assert CustomCodec.decode("127.0.0.1", %{"type" => "ip"}) == {127, 0, 0, 1}
    end

    test "an added type is cast by the new clause" do
      field = %{"type" => "scaled_float", "scaling_factor" => 100}

      assert CustomCodec.decode(1234, field) == 12.34
      assert CustomCodec.encode(12.34, field) == 1234
    end

    test "a clause over a built-in type replaces that cast" do
      assert CustomCodec.decode("2026-08-11", %{"type" => "date"}) == {:raw, "2026-08-11"}
    end

    test "delegating inherits the nil short-circuit and the identity fallback" do
      assert CustomCodec.decode(nil, %{"type" => "ip"}) == nil
      assert CustomCodec.decode("hello", %{"type" => "text"}) == "hello"
    end
  end

  describe "a mapping entry with no type" do
    test "falls back to identity in both directions" do
      assert Codec.decode("hello", nil) == "hello"
      assert Codec.encode("hello", %{"properties" => %{}}) == "hello"
    end
  end
end
